...
public class KickBackAbility : CharacterAbility
{
    ...
    private InputAction       kickBackAction;
    ...
    protected override void Start()
    {
        ...
        kickBackAction = playerInput.actions["Skill-1"];
        ...
        if (playerStats.currentMana >= skillData.manaCost)
        {
            ...
            switch (state)
            {
                case AbilityState.ready:
                    if (kickBackAction.IsPressed() && !runAction.IsPressed() && timer <= 0 && skillData.isUpgrade == true)
                    {
    ...
    {
        GetComponent<PlayerAudioManager>().PlayerSound("KickBack");
    }
